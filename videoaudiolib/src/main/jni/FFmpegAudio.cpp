...
int getPcm(FFmpegAudio *audio) {
    ...
    while(audio->isPlay){
        ...
        if (got_frame){
            ...
            //                缓冲区的大小
            size = av_samples_get_buffer_size(NULL,audio->out_channer_nb, frame->nb_samples,AV_SAMPLE_FMT_S16, 1);
            break;
        }
    }
    av_free(packet);
...
//        double time = datalen/((double) 44100 *2 * 2);
        double time  = datalen/((44100*2*2.0));
 ...
 */
    swr_alloc_set_opts( audio->swrContext,
                        out_ch_layout, out_formart, out_sample_rate,
                        audio->codec->channel_layout, audio->codec->sample_fmt, audio->codec->sample_rate,
                        0,
                        NULL);
...
int FFmpegAudio::createPlayer() {
    ...
    result = (* outputMixEnvironmentalReverb) ->SetEnvironmentalReverbProperties(outputMixEnvironmentalReverb,&settings);
